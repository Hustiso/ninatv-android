...
import com.nina.tv.ui.screens.account.InputField
...
private fun DebugDialogButton(
    ...
    Card(
        ...
        Text(
            text = text,
            style = MaterialTheme.typography.bodyMedium,
            color = if (isFocused) NinaColors.TextPrimary else NinaColors.TextSecondary,
            modifier = Modifier
                .fillMaxWidth()
    ...
    Column(
        ...
        if (result != null) {
            Text(
                text = result,
                style = MaterialTheme.typography.bodySmall,
                color = if (result.startsWith("Failed")) NinaColors.Error else NinaColors.Secondary
            )
        }
