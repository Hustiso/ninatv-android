package com.nina.tv.core.plugin

import android.util.Log
import com.lagradost.cloudstream3.TvType
import com.nina.tv.core.plugin.cloudstream.toNinaType
import com.nina.tv.core.plugin.cloudstream.tvTypeFromString
...
import com.nina.tv.domain.model.ExternalPluginEntry
import com.nina.tv.domain.model.LocalScraperResult
import com.nina.tv.domain.model.PluginManifest
import com.nina.tv.domain.model.PluginRepository
import com.nina.tv.domain.model.RemotePluginInfo
import com.nina.tv.domain.model.RepositoryType
import com.nina.tv.domain.model.ScraperInfo
import com.nina.tv.domain.model.ScraperManifestInfo
...
class PluginManager @Inject constructor(
    ...
    private val pluginSyncService: com.nina.tv.core.sync.PluginSyncService,
    private val authManager: com.nina.tv.core.auth.AuthManager,
    ...
    private val httpClient = OkHttpClient.Builder()
        .dns(com.nina.tv.core.network.IPv4FirstDns())
    ...
    /**
     * Canonicalize a URL for deduplication. For NinaTV-style URLs (that don't end in .json),
    ...
    private fun canonicalizeRepoUrl(url: String): String {
        ...
        // Otherwise canonicalize as NinaTV manifest
     ...
     * Auto-detects format: tries NinaTV manifest first, then external repo format.
     */
    suspend fun addRepository(manifestUrl: String): Result<PluginRepository> = withContext(Dispatchers.IO) {
        try {
            ...
            // try external format first to avoid a wasted 404 on the NinaTV path.
            ...
            // Try NinaTV format (with canonicalized /manifest.json URL)
            val canonicalManifestUrl = canonicalizeManifestUrl(sanitizedUrl)
            Log.d(TAG, "Trying NinaTV manifest: $canonicalManifestUrl")

            val manifest = fetchManifest(canonicalManifestUrl)
            if (manifest != null) {
                return@withContext addNinaRepository(canonicalManifestUrl, manifest)
            }

            // If we haven't tried external format yet, try it now
            if (!isExplicitJsonFile) {
                Log.d(TAG, "NinaTV manifest not found, trying external format: $sanitizedUrl")
                val externalResult = externalRepoParser.tryParse(sanitizedUrl)
                if (externalResult != null) {
                    return@withContext addExternalRepository(sanitizedUrl, externalResult)
                }
    ...
    private suspend fun addRepositoryWithTypeHint(
        ...
        try {
            ...
            when (typeHint) {
                ...
                RepositoryType.NINA_JS -> {
                    Log.d(TAG, "addRepositoryWithTypeHint: NINA_JS hint, trying manifest: $sanitizedUrl")
                    val canonicalManifestUrl = canonicalizeManifestUrl(sanitizedUrl)
                    val manifest = fetchManifest(canonicalManifestUrl)
                    if (manifest != null) {
                        return@withContext addNinaRepository(canonicalManifestUrl, manifest)
                    }
                    Log.w(TAG, "addRepositoryWithTypeHint: NINA_JS hint failed, falling back to auto-detect")
                }
    ...
    private suspend fun addNinaRepository(
        ...
        val repo = PluginRepository(
            ...
            type = RepositoryType.NINA_JS
        )

        dataStore.addRepository(repo)
        downloadJsScrapers(repo.id, canonicalManifestUrl, manifest.scrapers)

        Log.d(TAG, "NinaTV repository added: ${repo.name} with ${manifest.scrapers.size} scrapers")
        triggerRemoteSync()
    ...
    private suspend fun addExternalRepository(
        repoUrl: String,
        parseResult: com.nina.tv.core.plugin.cloudstream.ExternalRepoParseResult
    ): Result<PluginRepository> {
    ...
    suspend fun executeScraper(
        ...
        return when (scraper.type) {
            ...
            RepositoryType.NINA_JS -> executeJsScraper(scraper, tmdbId, mediaType, season, episode)
        }
    }
