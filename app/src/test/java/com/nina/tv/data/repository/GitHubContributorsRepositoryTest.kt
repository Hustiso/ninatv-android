...
import com.nina.tv.data.remote.dto.GitHubContributorDto
import com.nina.tv.data.remote.dto.GitHubReleaseDto
...
class GitHubContributorsRepositoryTest {
        ...
        val repository = GitHubContributorsRepository(
            gitHubApi = FakeGitHubApi(
                contributorsByRepo = mapOf(
                    ...
                    "ninamobile" to errorResponse(500)
                )
            )
        )
    ...
    fun `fails when both repositories fail`() = runTest {
        val repository = GitHubContributorsRepository(
            gitHubApi = FakeGitHubApi(
                contributorsByRepo = mapOf(
                    "NinaTV" to errorResponse(500),
                    "ninamobile" to errorResponse(503)
                )
            )
        )
